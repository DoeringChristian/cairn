...
def _seed(repo, n=3) -> dict[str, str]:
    ...
    for i in range(n):
        with cairn.Run(project="p", name=f"r{i}", repo=repo, **_RUN_KW) as run:
            ...
            run.track(float(i), name="val.loss", step=0)
...
def test_runquery_history_long_format(repo):
    ...
    assert list(df.columns) == ["run_id", "run_name", "name", "step", "wall_time", "value"]
    ...
    assert r1.sort_values("step").value.tolist() == [10.0, 11.0, 12.0]
    val = df[(df.run_id == ids["r1"]) & (df.name == "val.loss")]
    assert val.value.tolist() == [1.0]

    with cairn.Reader(repo) as reader:
        only = reader.runs("p").filter(lr__gt=0.25).history(keys=["val.loss"])
...
def test_run_history_wide(repo):
    ...
    with cairn.Reader(repo) as reader:
        ...
        wide = run.history()
        assert sorted(wide.columns) == ["acc", "loss", "val.loss"]
        assert wide["loss"].tolist() == [0.0, 1.0, 2.0] and wide.loc[0, "val.loss"] == 0.0
...
def test_compare_all_names_with_wall_time(client):
    ...
    assert set(by_name["b"][0]) == {"step", "wall_time", "value"}
...
def test_export_project_csv_with_filter(live_server, monkeypatch, tmp_path):
    ...
    assert list(rows[0]) == ["run_id", "run_name", "name", "step", "wall_time", "value"]
    assert {r["run_name"] for r in rows} == {"r1", "r2"}
    assert len(rows) == 2 * 5
    assert {r["name"] for r in rows} == {"loss", "val.loss", "acc"}
