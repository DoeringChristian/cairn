/**
 * Cairn JS plugin: Heatmap viewer using Canvas 2D.
 *
 * Expects artifact data as a flat Float32Array with metadata:
 *   { "rows": N, "cols": M, "label": "optional title" }
 *
 * Renders a color-mapped heatmap with value annotations.
 */
...
window.cairn.render = function (msg) {
  ...
  ctx.textAlign = "center";
  for (let c = 0; c < cols; c++) {
    ctx.fillText(String(c), pad + c * cellSize + cellSize / 2, height - 8);
  }
  ctx.textAlign = "right";
  for (let r = 0; r < rows; r++) {
    ctx.fillText(String(r), pad - 6, pad + r * cellSize + cellSize / 2 + 3);
  }
};
