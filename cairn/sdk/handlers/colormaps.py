"""Named colormaps baked into single-channel images (cairn.Image(x, colormap=...)).

Stops match the viewer's former colormaps: turbo and magma are sequential
over [0, 1]; red-blue and red-green are diverging over [-1, 1] with white
at zero. Values map by dtype like every image (float [0, 1] or [-1, 1] for
diverging maps), or over an explicit vmin/vmax; outside values clip.
"""
...
COLORMAPS = tuple(_STOPS)
DIVERGING = frozenset({"red-green", "red-blue"})


def default_range(name: str) -> tuple[float, float]:
    """The fixed range a colormap spans without ``vmin``/``vmax``: the same for
    every image and step, so one value is always one color."""
    return (-1.0, 1.0) if name in DIVERGING else (0.0, 1.0)


def apply_colormap(
    arr: np.ndarray, name: str, *, vmin: float | None = None, vmax: float | None = None
) -> np.ndarray:
    """Map a single-channel array (HW or HW1) to an HW3 uint8 RGB image."""
    if name not in _STOPS:
        raise ValueError(f"colormap must be one of {COLORMAPS}, got {name!r}")
    if arr.ndim == 3 and arr.shape[-1] == 1:
        arr = arr[..., 0]
    if arr.ndim != 2:
        raise ValueError(f"colormap={name!r} needs a single-channel image, got shape {arr.shape}")
    lo, hi = default_range(name)
    lo = lo if vmin is None else float(vmin)
    hi = hi if vmax is None else float(vmax)
    if not hi > lo:
        raise ValueError(f"colormap range needs vmax > vmin, got [{lo}, {hi}]")
    values = np.nan_to_num(arr.astype(np.float64), nan=lo, posinf=hi, neginf=lo)
    t = np.clip((values - lo) / (hi - lo), 0.0, 1.0)
    # Interpolate the stops directly (no 256-entry table), so a diverging map's
    # middle stop — zero — is exactly white.
    stops = np.asarray(_STOPS[name], dtype=np.float64)
    at = np.linspace(0.0, 1.0, len(stops))
    rgb = np.stack([np.interp(t, at, stops[:, c]) for c in range(3)], axis=-1)
