...
**2026-07-17 update: the WebGL2 backend described throughout this spec was removed.** The engine is now WebGPU-only; see the "Fallback" bullet below for the current fallback boundary. The rest of this document's WebGL2 mentions are historical (describe what was originally built, then deleted) — not current architecture.
...
- **Fallback (UPDATED 2026-07-17 — WebGL2 backend removed):** the engine is **WebGPU-ONLY**. The original "WebGPU primary + WebGL2 reduced fallback in one engine" decision (below, struck through) is superseded: when WebGPU is unavailable, the fallback is the EXISTING legacy CPU/2D-canvas pane (`ImagePane`/`HdrImagePane`/`CompositeMediaPane`), selected via a capability-gated `resolveImageRenderer` seam (`plot-renderers.tsx`) up front, plus the C1 error boundary (`GpuImagePane`/`GpuComparePane` self-heal to the legacy pane on a runtime GPU failure) as the runtime safety net. WebGL2 was SDR-only and was the source of the Y-flip correction, the park/restore-on-context-lost machinery (FU1), and live-context-exhaustion issues — removing it simplifies the engine to one backend and makes cairn-plot easier to publish as a standalone lib. ~~WebGPU primary + WebGL2 fallback in ONE engine (one abstraction/scene graph we own; no three.js). WebGL2 is a reduced path: no compute, no HDR — but it renders the full feature set in SDR so shared self-contained notebooks display for everyone. WebGPU = full (HDR, compute image ops, advanced volume, GPU picking); WebGL2 = graceful reduced.~~
