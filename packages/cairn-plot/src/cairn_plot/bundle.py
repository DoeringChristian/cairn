...
def _resolve_inline_dir() -> Path:
    ...
    2. a repo-checkout fallback — ``cairn/ui/dist/plot-inline`` relative to a
       source tree — so an editable/dev checkout that has not run the asset-sync
       step still finds the freshly-built dist.
    """
    try:
    ...
    # ``packages/cairn-plot/src/cairn_plot/bundle.py``; walk up to the repo root
    # and read the committed viewer dist.
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "cairn" / "ui" / "dist" / "plot-inline"
